...
options:
  url:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_URL) environment variable.
    type: str
  urls:
    description:
      - Elasticsearch base URLs used in order for request failover.
      - Mutually exclusive with I(url).
      - Can also be set as a comma-separated C(ELASTICSEARCH_URLS) environment variable.
    type: list
    elements: str
  username:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_USERNAME) environment variable.
    type: str
  password:
    description:
      - Password for HTTP basic authentication.
      - Must be supplied together with I(username).
      - Can also be set with the C(ELASTICSEARCH_PASSWORD) environment variable.
    type: str
  api_key:
    description:
      - Encoded Elasticsearch API key.
      - Mutually exclusive with basic authentication and I(bearer_token).
      - Can also be set with the C(ELASTICSEARCH_API_KEY) environment variable.
    type: str
  bearer_token:
    description:
      - Bearer token used for authentication.
      - Mutually exclusive with basic authentication and I(api_key).
      - Can also be set with the C(ELASTICSEARCH_BEARER_TOKEN) environment variable.
    type: str
  headers:
    description:
      - Additional HTTP headers added to every request.
      - Authentication options override an C(Authorization) value in this mapping.
      - Can also be set as a JSON mapping in the C(ELASTICSEARCH_HEADERS) environment variable.
    type: dict
    default: {}
  validate_certs:
    description:
      - Whether to validate the Elasticsearch TLS certificate.
      - Can also be set with the C(ELASTICSEARCH_VALIDATE_CERTS) environment variable.
    type: bool
    default: true
  ca_path:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_CA_PATH) environment variable.
    type: path
  ca_data:
    description:
      - PEM CA certificate data used for server verification.
      - Mutually exclusive with I(ca_path).
      - Can also be set with the C(ELASTICSEARCH_CA_DATA) environment variable.
    type: str
  client_cert:
    description:
      - Path to a PEM client certificate chain.
      - The file can include its private key; otherwise also set I(client_key).
      - Can also be set with the C(ELASTICSEARCH_CLIENT_CERT) environment variable.
    type: path
  client_key:
    description:
      - Path to the PEM private key for I(client_cert).
      - Can also be set with the C(ELASTICSEARCH_CLIENT_KEY) environment variable.
    type: path
  certificate_fingerprint:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_CERTIFICATE_FINGERPRINT) environment variable.
    type: str
  timeout:
    description:
      - Request timeout in seconds.
      - Can also be set with the C(ELASTICSEARCH_TIMEOUT) environment variable.
    type: int
    default: 30
  retries:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_RETRIES) environment variable.
    type: int
    default: 3
  retry_pause:
    description:
      - Initial retry delay in seconds. The delay doubles for each attempt, up to 60 seconds.
      - Can also be set with the C(ELASTICSEARCH_RETRY_PAUSE) environment variable.
    type: float
    default: 1.0
  retry_status_codes:
    description:
  ...
  retry_mutating_requests:
    description:
      ...
      - Can also be set with the C(ELASTICSEARCH_RETRY_MUTATING_REQUESTS) environment variable.
    type: bool
    default: false
  url_username:
    description:
